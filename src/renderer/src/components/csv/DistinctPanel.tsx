...
// the CSV grid stays visible. Exports to a notepad tab: distinct values, or every value.
...
export function DistinctPanel({
  ...
  return (
    <aside className="csv-distinct flex flex-col w-72 shrink-0 border-l border-citrus-border bg-citrus-cream dark:border-citrus-night-border dark:bg-citrus-night">
      ...
      <div className="flex gap-1.5 px-3 py-2 border-b border-citrus-border/60 dark:border-citrus-night-border/60">
        ...
        >
          <ArrowRightLeft className="w-3 h-3" /> Distinct → Notepad
        </button>
        <button
      ...
      <div className="flex-1 overflow-auto scrollbar-none">
        {rows.map((r) => (
          <div
            key={r.val}
            className="w-full flex items-center justify-between gap-2 px-3 py-1 text-[11px] font-mono"
          >
        ...
        {truncated && (
          <div className="px-3 py-2 text-[10px] text-citrus-muted dark:text-citrus-night-muted">
            showing top {DISTINCT_LIMIT} — use “All” to export every value
          </div>
        )}
      </div>
    </aside>
